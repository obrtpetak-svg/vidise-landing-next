...
export function Navbar() {
    ...
    return (
        <motion.nav initial={{ y: -80 }} animate={{ y: 0 }} transition={{ duration: 0.6, ease: [0.16, 1, 0.3, 1] }}
            className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${scrolled ? "bg-[#0A0A0F]/90 backdrop-blur-2xl border-b border-white/[0.06] shadow-2xl shadow-black/20" : "bg-transparent"}`}>
            ...
            <AnimatePresence>
                {mobileOpen && (
                    <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} exit={{ opacity: 0, height: 0 }}
                        className="md:hidden bg-[#0A0A0F]/95 backdrop-blur-2xl border-t border-white/[0.06] px-6 pb-6">
...
function NavLink({ href, children, onClick }: { href: string; children: ReactNode; onClick?: () => void }) {
    return <Link href={href} onClick={onClick} className="text-sm font-medium text-slate-400 hover:text-white transition-colors">{children}</Link>;
}
                ...
                <div className="flex flex-col md:flex-row justify-between items-center gap-6">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <div className="text-xs text-slate-500">Workforce Management Platform</div>
                        </div>
                    </div>
                    <div className="flex items-center gap-6 text-sm text-slate-500">
                        <Link href="/#features" className="hover:text-white transition-colors">Mogućnosti</Link>
                        <Link href="/#how" className="hover:text-white transition-colors">Kako radi</Link>
                        <a href="mailto:info@vi-di.me" className="hover:text-white transition-colors">Kontakt</a>
                        <a href="https://www.vi-di.me" target="_blank" className="hover:text-white transition-colors">Vi-Di.me</a>
                    </div>
                </div>
                <div className="mt-8 pt-6 border-t border-white/[0.06] text-center text-xs text-slate-600">
